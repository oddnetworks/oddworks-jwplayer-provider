...
class Client {
	...
	getVideosByPlaylist(args) {
		...
		args.query = {
			format: 'json',
			poster_width: 1920 // eslint-disable-line camelcase
		};
		return this.makeRequest(args);
	}
	...
	getVideoTracks(args) {
		const id = args.videoId;
		if (!id || typeof id !== 'string') {
			throw new Error('getVideoTracks() videoId is required');
		}
		args.path = `${PATH_PREFIX}/videos/tracks/list`;
		args.query = {
			video_key: id // eslint-disable-line camelcase
		};
		return this.makeRequest(args);
	...
	searchVideos(args) {
		...
		args.query = Object.assign(
			query, {
				...
				statuses_filter: 'ready' // eslint-disable-line camelcase
			}
		);
		args.path = `${PATH_PREFIX}/videos/list`;
		return this.makeRequest(args);
	...
	searchPlaylists(args) {
		...
		args.query = Object.assign(query, {
			search
		});
		args.path = `${PATH_PREFIX}/channels/list`;
		return this.makeRequest(args);
	...
	makeRequest(args) {
		...
		if (pathParts[1] === 'v2') {
			...
			qs = Object.assign({}, query, {
				api_signature: signature // eslint-disable-line camelcase
			});
		}
		...
		return new Promise((resolve, reject) => {
			request(params, (err, res, body) => {
				...
				if (res.statusCode !== 200) {
					return reject(new Boom(res.statusMessage, {
	...
	static generateToken(params) {
		...
		if (params.related_media_id) {
			payload.related_media_id = params.related_media_id; // eslint-disable-line camelcase
		}

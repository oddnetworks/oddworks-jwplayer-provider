...
class Client {
	...
	constructor(spec) {
		...
		this.v2getVideosByPlaylist = this.v2getVideosByPlaylist.bind(this);
	...
	// args.playlistId *required
	v2getVideosByPlaylist(args) {
		const id = args.playlistId;
		if (!id || typeof id !== 'string') {
			throw new Error('v2getVideosByPlaylist() playlistId is required');
		}
		const V2_PREFIX = `/v2`;
	...
	searchVideos(args) {
		return this.getPlaylists(args)
		.then(res => {
			const channels = res.channels || [];
			if (channels.length === 0) {
				throw new Error('searchVideos() could find no channels/playlists');
			}
			const searchChannels = channels.filter(channel => {
				return channel.type.toLowerCase() === 'search';
			});

			if (searchChannels.length === 0) {
				throw new Error('searchVideos() could not find a search channel/playlist');
			}

			const playlistId = searchChannels[0].key || '';

			if (playlistId.length === 0) {
				throw new Error('searchVideos() could not find the key/playlistId for the search channel/playlist');
			}
			...
			const V2_PREFIX = `/v2`;
			args.path = `${V2_PREFIX}/playlists/${playlistId}`;
			args.secretKey = this.secretKey;
		...
		.then(res => {
			console.log('PLAYLIST LENGTH: ', res.playlist.length);
			return res;
		});
	}
	...
	searchPlaylists(args) {
		...
		args.query = Object.assign({}, {search});
		args.path = `${PATH_PREFIX}/channels/list`;
		return this.makeRequest(args);
	}
